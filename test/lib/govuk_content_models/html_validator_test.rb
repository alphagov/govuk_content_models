...
class HtmlValidatorTest < ActiveSupport::TestCase
  should "allow Govspeak Markdown" do
    ...
    values.each do |value|
      assert HtmlValidator.new(value).valid?
    end
  end

  should "disallow a script tags" do
    assert HtmlValidator.new("<script>alert('XSS')</script>").invalid?
  end
  ...
  should "disallow a javascript protocol in an attribute" do
    ...
    assert HtmlValidator.new(html).invalid?
  end
  ...
  should "allow non-JS HTML content" do
    assert HtmlValidator.new("<a href='foo'>").valid?
  end

  should "allow things that will end up as HTML entities" do
    assert HtmlValidator.new("Fortnum & Mason").valid?
  end
end